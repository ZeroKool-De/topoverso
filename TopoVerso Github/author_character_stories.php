...
// Parametri ID dell'autore e del personaggio
$person_id = isset($_GET['person']) ? (int)$_GET['person'] : 0;
$character_id = isset($_GET['character']) ? (int)$_GET['character'] : 0;

if (!$person_id || !$character_id) {
    header('Location: authors_page.php?message=Parametri non validi per la ricerca');
    exit;
}
...
if (!$author || !$character) {
    header('Location: authors_page.php?message=Autore o personaggio non trovato');
    exit;
}
...
// Query per le storie con questo autore e personaggio
$order_by_sql = "";
switch ($current_sort) {
    case 'date_asc':
...
$sql_count = "
    ...
    WHERE sp.person_id = ? AND sc.character_id = ?
";
$stmt_count = $mysqli->prepare($sql_count);
$stmt_count->bind_param("ii", $person_id, $character_id);
$stmt_count->execute();
...
$sql_stories = "
SELECT DISTINCT s.story_id, s.title, s.story_title_main, s.part_number, s.total_parts,           s.sequence_in_comic, s.created_at as story_created_at, s.first_page_image,
...
$stmt_stories->bind_param("iiii", $person_id, $character_id, $items_per_page, $offset);
...
$page_title = htmlspecialchars($author['name']) . " e " . htmlspecialchars($character['name']);

require_once 'includes/header.php';
?>

<div class="container page-content-container">
    <div class="author-character-header">
        ...
        <p class="stories-summary">
            ...
            storie in cui <?php echo htmlspecialchars($author['name']); ?> ha lavorato con il personaggio <?php echo htmlspecialchars($character['name']); ?>
        </p>
    </div>

    <?php if ($total_stories > 0): ?>
        <div class="controls-bar">
            <form method="GET" action="author_character_stories.php" class="form-inline">
        ...
        <?php if ($total_pages > 1): ?>
            <div class="pagination-controls">
                ...
                $base_url = 'author_character_stories.php?' . http_build_query($base_url_params);
                $separator = '&';
                ?>
    ...
    <?php else: ?>
        <div class="no-stories">
            <p>Nessuna storia trovata in cui <?php echo htmlspecialchars($author['name']); ?> ha lavorato con il personaggio <?php echo htmlspecialchars($character['name']); ?>.</p>
        </div>
    <?php endif; ?>
